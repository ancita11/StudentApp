...
public class AccountantDB {
	...
	public static int save(AccountantBean bean){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement( " insert into accountant(name,email,password,address,contact) "
					+ " values(?,?,?,?,?) " );
	...
	public static int update(AccountantBean bean){
		...
		try{
			Connection con=ConnectionProvider.giveConnection();
			PreparedStatement ps=con.prepareStatement(" update accountant "
					+ " set name=?,email=?,password=?,address=?,contact=? "
					+ " where id=? ");
	...
	public static int delete(int id){
		...
		try{
			Connection con=ConnectionProvider.giveConnection();
			PreparedStatement ps=con.prepareStatement(" delete from accountant "
					+ " where id=? ");
			ps.setInt(1,id);
